...
# Pure prompt theme path (commented alternative path left for reference)
...
# Set up LS_Colors based on https://github.com/trapd00r/LS_COLORS/blob/master/lscolors.sh
...
# Customize Pure prompt colors based on user (red for root, blue for others) and hostname (yellow)
if [[ $(id -u) -eq 0 ]]; then
...
# Keybindings for faster navigation: Ctrl + Arrow keys to move by word, Ctrl + Backspace to delete words
...
# Add games to PATH and create a fun alias with `fortune` and `cowsay` if installed
